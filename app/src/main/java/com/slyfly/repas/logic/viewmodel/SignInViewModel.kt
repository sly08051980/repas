...
class SignInViewModel(
    ...
    fun onIntent(intent: SignInItent) {
        when (intent) {
            is SignInItent.ChangeEmail ->
                _uiState.value = _uiState.value.copy(email = intent.value.trim(), error = null)
    ...
    private fun submit() {
        ...
        if (s.email.isBlank() || s.password.isBlank()) {
            _uiState.value = s.copy(error = "Tous les champs doivent être remplis")
            return
        }
        ...
        viewModelScope.launch {
            val result = signInUserUseCase(s.email.lowercase(), s.password)
            ...
            result.fold(
                onSuccess = {
                    ...
                    println(">>> SUCCESS = ${_uiState.value.success}")
                },
                onFailure = {
                    _uiState.value = _uiState.value.copy(
                        isLoading = false,
                    ...
                    println(">>> ERROR = ${_uiState.value.error}")
                }
            )
